...
histIdade <- sqldf("SELECT AGE, COUNT(AGE) AS count_age FROM dados
                    GROUP BY AGE
...
arrange(gasto_total_idade, desc(Gasto_Total))
arrange(gasto_total_idade, desc(Gasto_Total))[1,]


maior_gasto_total_idade <- sqldf('
                           SELECT AGE as Idade, 
                           SUM(TOTCHG) as Gasto_Total 
                           FROM dados 
                           GROUP BY AGE
...
barplot(tapply(gasto_total_baseado_idade$TOTCHG, 
               ...
               FUN = sum),
        main = "Distribuição de Gastos por Idade",
        xlab = "Idade", 
        border = "black", 
        col = c("light gray", "dark gray"), 
        xlim = c(0,22), 
...
which.max(gasto_total_baseado_diag$TOTCHG) 
...
modelo_anova_3 <- aov(TOTCHG ~ FEMALE, data = dados)
summary(modelo_anova_3)

# Resposta da Pergunta 5:
# O valor-p*2 é menor que 0.05 para AGE. Rejeitamos a hipótese nula para esse variável independente.
# O valor-p*2 é maior que 0.05 para FEMALE. Não podemos rejeitamos a hipótese nula para esse variável independente.

# Há um efeito significativo da idade nos custos hospitalares, mas esse efeito não é tão significativo para variável genero.
